...
probOcc_log_odds = prob_to_log_odds(probOcc);
probFree_log_odds = prob_to_log_odds(probFree);
for sc=1:columns(laserEndPntsMapFrame)
  ...
  ls_x = laserEndPntsMapFrame(1,sc);
  ls_y = laserEndPntsMapFrame(2,sc);
  rb_x = robPoseMapFrame(1);
  rb_y = robPoseMapFrame(2);
  [X,Y] = bresenham([rb_x, rb_y; ls_x, ls_y]);
...
for k=1:columns(freeCells)
  x = freeCells(1,k);
  y = freeCells(2,k);
  mapUpdate(x,y) = probFree_log_odds;
endfor

for sc=1:columns(laserEndPntsMapFrame)
  %TODO: update the log odds values in mapUpdate for each laser endpoint according to probOcc.
  % for some reason including this in the previous 'sc' loop was not working...
  x = laserEndPntsMapFrame(1,sc);
  y = laserEndPntsMapFrame(2,sc);
  mapUpdate(x,y) = probOcc_log_odds;
endfor
