...
for i = 1:m
  ...
  z_points(1,:) = sqrt(delta_x.^2 + delta_y.^2);
  z_points(2,:) = normalize_angle(atan2(delta_y, delta_x) - sigma_points(3,:));
  ...
  zym = 0;
  for k=1:(2*n+1)
    zm(1) = zm(1) + wm(k) * z_points(1,k);
    expected_phi = z_points(2,k);
    zxm = zxm + wm(k) * cos(expected_phi);
    zym = zym + wm(k) * sin(expected_phi);
  endfor
  zm(2) = normalize_angle(atan2(zym,zxm));
  ...
  for k=1:(2*n+1)
    ...
    dx = sigma_points(:,k) - mu;
    dx(3) = normalize_angle(dx(3));
    Sigma_x_z += wc(k)*(dx * dz');
  endfor
  S = S + Q;
  ...
  z_innov = z_actual - zm;
  z_innov(2) = normalize_angle(z_innov(2));
  mu = mu + K*z_innov;
  sigma = sigma - K * S * K';
